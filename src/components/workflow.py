...
class BasicWorkflow:
    """
    This module is a higher level abstraction of the classes contained within the module `incose` 
    and is called in the main program script `aiswre`. The module contains the class `BasicWorkflow`
    which contains the following methods: `preprocess_data`, `load_requirements`, `revise_requirements`, 
    and `save_output`. The `preprocess` method instantiates the `PreprocessIncoseGuide` class to 
    prepare the incose guide for analysis. Next, templates are built using the preprocessed guide via 
    instantiation of the `BuildIncoseTemplates` class. To appropriately tie the templates with specific 
    evaluation functions, the `BuildIncoseEvalConfig` class is instantiated. The method `load_requirements` 
    loads into memory the requirements dataset (Excel file) to be analyzed. The AI review occurs in method 
    `revise_requirements` which calls the `run_eval_loop` function. This function is preceeded and 
    followed by calling of the `run_eval_sequence` method from the `IncoseRequirementReviewer` class. 
    The `run_eval_sequence` is called before and after `run_eval_loop` to capture metrics associated with 
    how well the revision performed against known evaluation criteria (based on the INCOSE Guide). Lastly, 
    the `save_output` method collects all intermediate outputs and saves them into a dedicated run folder. 
    The `save_output` method also writes the results to `results.xlsx` which may be thought of as a "journal" 
    of all results generated by running the program.
